...
class EraserCubit extends Cubit<EraserState> {
  ...
  EraserCubit(this._removeBackgroundUseCase) : super(const EraserInitial());
  ...
  Future<void> removeBackground(File image) async {
    ...
    result.fold(
      (failure) {
        ...
        getIt<Talker>().error('Failed to remove background: $errorMessage');
        emit(EraserError(errorMessage));
      },
      (processedImage) {
        _processedImage = processedImage;
        getIt<Talker>().info(
          'Background removed successfully: ${processedImage.path}',
        );
        emit(
          EraserSuccess(
  ...
  void undo() {
    if (state is EraserSuccess && _originalImage != null) {
      emit(const EraserInitial());
      getIt<Talker>().info('Undo: showing original image');
    }
  }
  ...
  void redo() {
    if (state is EraserInitial &&
      ...
      getIt<Talker>().info('Redo: showing processed image');
    }
  }
